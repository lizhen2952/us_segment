...
class Images_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(self.images_dir)):
            image = io.imread(self.images_dir[i], as_gray=False, pilmode="RGB")
            label = io.imread(self.labels_dir[i], as_gray=True)
            if self.transformI:
...
class Images_Dataset_folder(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        seed=np.random.randint(0,2**32,dtype=np.int64) # make a seed with numpy generator 
